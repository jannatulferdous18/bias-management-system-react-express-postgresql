...
app.post('/login', async (req, res) => {
  ...
  app.post('/register', async (req, res) => {
    ...
    try {
      const userExists = await db.query('SELECT * FROM users WHERE user_name = $1', [user_name]);
  
      if (userExists.rows.length > 0) {
        return res.status(409).json({ success: false, message: 'User name already exists, try another name'});
      }
  
      await db.query('INSERT INTO users (user_name, password) VALUES ($1, $2)', [user_name, password]);
  ...
  app.get('/admin/pending-biases', async (req, res) => {
    try {
      ...
      console.error(err);
      res.status(500).json({ success: false, message: 'Could not fetch pending biases' });
    }
  });
  ...
  app.post('/admin/approve-bias', async (req, res) => {
    ...
    try {
      // 1. Get pending bias
      const result = await db.query('SELECT * FROM pending_biases WHERE bias_request_id = $1', [id]);
      const bias = result.rows[0];
  
      if (!bias) {
        return res.status(404).json({ success: false, message: 'Bias not found' });
      }
  
      // 2. Insert into biases
      ...
      // 3. Insert into mitigation_strategies
      ...
      // ✅ 4. Update biases table with m_strategy_id
      await db.query(
        `UPDATE biases 
         SET m_strategy_id = $1
         WHERE bias_id = $2`,
      ...
      await db.query('DELETE FROM pending_biases WHERE bias_request_id = $1', [id]);
  
      res.json({ success: true, message: 'Bias approved and linked to mitigation strategy' });
...
app.get('/api/bias-types', async (req, res) => {
  try {
    const result = await db.query('SELECT DISTINCT bias_type FROM biases ORDER BY bias_type ASC');
...
app.post('/api/biases/admin', async (req, res) => {
  ...
  try {
    ...
    const userResult = await db.query('SELECT user_id FROM users WHERE user_name = $1', [submittedBy]);
    ...
    const result = await db.query(
      `INSERT INTO biases (bias_type, bias_source, bias_description, severity_score, affected_groups, submitted_by)
       VALUES ($1, $2, $3, $4, $5, $6) RETURNING bias_id`,
      [biasType, biasSource, description, severity, affectedGroups, userId]
    );

    const newBiasId = result.rows[0].bias_id;
    ...
    res.status(201).json({ success: true, message: 'Bias submitted successfully' });
  } catch (err) {
    console.error('Error in /api/biases/admin:', err);
    res.status(500).json({ success: false, message: 'Admin submission failed' });
  }
});
...
app.put('/admin/biases/:id', async (req, res) => {
  ...
  try {
    ...
    if (strategyCheck.rows.length > 0) {
      ...
      // 2B. Strategy does not exist → insert new one
      const strategyRes = await db.query(
        `INSERT INTO mitigation_strategies (bias_id, m_strategy_description)
      ...
      // 3. Update m_strategy_id in biases table
      await db.query(
